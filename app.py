    ...
    if len(opened_list) > 0:
        for i in range(len(opened_list)):
            opened_rows_html += f"""<tr>
                ...
                <td>{str(datetime.strptime(str(opened_list[i]['Date & Time Sent']), "%Y-%m-%d %H:%M:%S%z").strftime("%d/%m/%Y %I:%M %p"))}</td>
                <td>{str(datetime.strptime(str(opened_list[i]['Opened Date & Time']), "%Y-%m-%d %H:%M:%S%z").strftime("%d/%m/%Y %I:%M %p"))}</td>
            </tr>"""
    ...
    if len(closed_list) > 0:
        for i in range(len(closed_list)):
            closed_rows_html += f"""<tr>
                ...
                <td>{str(datetime.strptime(str(closed_list[i]['Date & Time Sent']), "%Y-%m-%d %H:%M:%S%z").strftime("%d/%m/%Y %I:%M %p"))}</td>
            </tr>"""
